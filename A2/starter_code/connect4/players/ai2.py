...
class AIPlayer:
    ...
    def get_expectimax_move(self, state: Tuple[np.array, Dict[int, Integer]]) -> Tuple[int, bool]:
        ...
        for col in range(columns) :
            ...
            new_state_pop = self.state_update(state, col, 1, self.player_number)
            if new_state_push != -1:
                val = self.get_expectimax_score(new_state_push, 3-self.player_number, 4)
                if val > maax :
                    maax = val
                    move = col
                    isPop = 0 
            if new_state_pop != -1:
                val = self.get_expectimax_score(new_state_pop, 3-self.player_number, 4)
                if val > maax :
                    maax = val
                    move = col
                    isPop = 1
    ...
    def evaluate(self, state: Tuple[np.array, Dict[int, Integer]]) -> int :
        return get_pts(self.player_number, state[0])/max(1, state[1][3-self.player_number].get_int())-get_pts(3-self.player_number, state[0])/max(1, state[1][self.player_number].get_int())